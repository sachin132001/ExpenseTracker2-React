...
export default function UpdatePayment() {
  ...
  useEffect(() => {
    const fetchPayment = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/Pupdate/${id}`);
        const { ptype, pmode, pdesc } = response.data;
        setPtype(ptype);
        setPmode(pmode);
        setPdesc(pdesc);
      } catch (error) {
  ...
  const handleSubmit= async(event)=>{
    ...
    try{
      const response= await axios.post(`http://localhost:8080/Pupdate/${id}`,{
        ptype,
        pmode,
        pdesc
      });
      ...
      alert("ERROR OCCURED DURING UPDATING PAYMENT")
    }
  }
  ...
  return (
    ...
    <div className="container mt-3">
		...
		<div className="col-md-4 jumbotron">
			...
			<h4 className="text-center bg-primary text-white">UPDATE PAYMENT</h4> 
		<br></br>
